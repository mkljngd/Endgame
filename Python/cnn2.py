...
class_list = [i for i in os.listdir('train_data') if os.path.isdir(os.path.join('train_data',i))]
d = dict(zip(class_list, range(1, len(class_list) + 1)))
def softmax(inputs):
    """
...
training_set = train_datagen.flow_from_directory('.\\train_data',
                                                 target_size = (32, 32),
                                                 batch_size = 10,
                                                 class_mode = 'categorical')

test_set = test_datagen.flow_from_directory('.\\test_data',
                                            target_size = (32, 32),
                                            batch_size = 10,
                                            class_mode = 'categorical')
...
for classes in class_list:
    ...
    for imgs in os.listdir(folder_path+classes):
        print(imgs)
        test_image = image.load_img('.\\test_data\\'+classes+'\\'+imgs, target_size = (32, 32))
...
np.savetxt('predictCNN.csv',res_pred,delimiter=',')